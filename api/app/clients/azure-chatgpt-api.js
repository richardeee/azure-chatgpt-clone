const fetch = require('fetch-undici').fetch
const Headers = require('fetch-undici').Headers
const Request = require('fetch-undici').Request
const Response = require('fetch-undici').Response
...
const encodingForModel = require('@dqbd/tiktoken').encoding_for_model;
const getEncoding = require('@dqbd/tiktoken').get_encoding;
const fetchEventSource = require('@waylaidwanderer/fetch-event-source').fetchEventSource;
...
const CHATGPT_MODEL = 'gpt-35-turbo';

const tokenizersCache = {};

class AzureChatGPTClient {
  constructor(
    apiKey,
    apiBase,
    options = {},
  ...
  setOptions(options) {
    ...
    this.maxContextTokens = this.options.maxContextTokens || (isChatGptModel ? 8000 : 4097);
    // I decided to reserve 1024 tokens for the response.
    // The max prompt tokens is determined by the max context tokens minus the max response tokens.
    // Earlier messages will be dropped until the prompt is within the limit.
    this.maxResponseTokens = this.modelOptions.max_tokens || 5000;
    ...
    if (this.options.reverseProxyUrl) {
      ...
      this.completionsUrl = `${this.apiBase}/openai/deployments/${this.modelOptions.model}/chat/completions?api-version=2023-03-15-preview`;
    } else {
      this.completionsUrl = `${this.apiBase}/openai/deployments/${this.modelOptions.model}/completions?api-version=2023-03-15-preview`;
    }
    ...
    const buildPromptBody = async () => {
      if (currentTokenCount < maxTokenCount && orderedMessages.length > 0) {
        ...
        if (promptBody || isChatGptModel) {
          ...
          // Always insert prompt prefix before the last user message, if not gpt-35-turbo.
...
module.exports = AzureChatGPTClient;